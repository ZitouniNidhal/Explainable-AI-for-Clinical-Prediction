...
st.set_page_config(
    page_title="XAI Clinical - Outcome Prediction", page_icon="🏥", layout="wide"
)
...
def load_model():
    ...
    preprocessor = joblib.load(
        Path(__file__).parent.parent / "models" / "preprocessor.joblib"
    )
...
def main():
    ...
    page = st.sidebar.radio(
        "Navigation", ["Prediction", "Explanations", "Robustness Analysis", "About"]
    )
...
def show_prediction_page():
    ...
    if st.button("Predict Risk", type="primary"):
            ...
            [
                {
                    ...
                    "surgery_type": (
                        ...
                        else 1 if surgery_type == "Major" else 2
                    ),
                    "blood_loss": 200,
                    "transfusion": 0,
                    "hypothermia": 0,
        ...
        try:
            ...
            with col_res1:
                st.metric(label="Complication Risk", value=f"{prediction_proba:.1%}")
            ...
            st.info(
                "Make sure to train the model with `python -m xai_clinical.pipeline`"
            )
...
def show_robustness_page():
    ...
    st.bar_chart(
        {"Noise level": ["1%", "5%", "10%"], "Jaccard Index": [0.92, 0.82, 0.71]}
    )
