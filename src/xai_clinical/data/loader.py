...
class DataLoader:
    ...
    def load_pancan_expression(config: dict) -> pd.DataFrame:
        ...
        if not os.path.exists(expression_path):
            raise FileNotFoundError(
                f"Fichier d'expression introuvable : {expression_path}"
            )
    ...
    def load_pancan_mutations(config: dict) -> pd.DataFrame:
        ...
        if not os.path.exists(mutations_path):
            raise FileNotFoundError(
                f"Fichier de mutations introuvable : {mutations_path}"
            )
    ...
    def load_mimic_sample(
        ...
        data = {
            ...
            "admission_type": np.random.choice(
                ["ELECTIVE", "EMERGENCY", "URGENT"], n_samples, p=[0.5, 0.3, 0.2]
            ),
            "length_of_stay": np.random.lognormal(2, 0.5, n_samples)
            .round()
            .astype(int),
            ...
            "heart_rate_mean": np.random.normal(85, 15, n_samples)
            .clip(40, 150)
            .round(1),
            ...
            "creatinine_max": np.random.normal(1.2, 0.5, n_samples)
            .clip(0.5, 5)
            .round(2),
            "bilirubin_max": np.random.exponential(0.8, n_samples)
            .clip(0.1, 10)
            .round(2),
            "platelet_min": np.random.normal(250, 80, n_samples)
            .clip(50, 500)
            .astype(int),
            "gcs_min": np.random.choice(
                [3, 6, 9, 12, 15], n_samples, p=[0.05, 0.1, 0.15, 0.3, 0.4]
            ),
            "ventilation_hours": np.random.exponential(12, n_samples)
            .clip(0, 168)
            .round(1),
    ...
    def load_uptodate_format(self, file_path: str) -> pd.DataFrame:
        ...
        try:
            ...
            logger.warning(
                "Standard parsing failed, attempting custom UptoDate parsing"
            )
