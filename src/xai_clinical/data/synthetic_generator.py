...
class SyntheticDataGenerator:
    ...
    def _get_feature_names(self) -> List[str]:
        ...
        return [
            ...
            'age', 'sex', 'bmi',
            # Medical history
            'diabetes', 'hypertension', 'heart_disease', 'renal_insufficiency',
            # Pre-operative parameters
            'hemoglobin', 'creatinine', 'leukocytes', 'platelets',
            'sodium', 'potassium', 'albumin',
            # Respiratory function
            'respiratory_rate', 'oxygen_saturation', 'pao2_fio2_ratio',
            # Cardiac function
            'heart_rate', 'systolic_bp', 'diastolic_bp',
            # Risk scores
            'asa_score', 'nyha_class',
            # Surgery characteristics
            'surgery_duration', 'emergency', 'surgery_type',
            # Peri-operative parameters
            'blood_loss', 'transfusion', 'hypothermia'
        ]
    ...
    def generate(self) -> Tuple[pd.DataFrame, pd.Series]:
        ...
        data['diabetes'] = (np.random.binomial(1, 0.3 + 0.01 * (data['age'] - 50).clip(0) / 10, n))
        data['hypertension'] = (np.random.binomial(1, 0.4 + 0.008 * (data['age'] - 55).clip(0) / 10, n))
        data['heart_disease'] = (np.random.binomial(1, 0.2 + 0.015 * (data['age'] - 60).clip(0) / 10, n))
        data['renal_insufficiency'] = (np.random.binomial(1, 0.1 + 0.01 * data['diabetes'], n))
        
        # 3. Biological parameters
        data['hemoglobin'] = np.random.normal(13, 2, n) - 0.5 * data['renal_insufficiency']
        data['creatinine'] = np.random.normal(80, 20, n) + 30 * data['renal_insufficiency'] + 10 * data['diabetes']
        ...
        data['respiratory_rate'] = np.random.normal(16, 3, n) + 2 * data['heart_disease']
        data['oxygen_saturation'] = np.random.normal(97, 2, n) - 3 * data['heart_disease']
        data['pao2_fio2_ratio'] = np.random.normal(300, 50, n) - 40 * data['heart_disease']
        ...
        # 6. Risk scores
        data['asa_score'] = np.random.choice([1, 2, 3, 4, 5], n, p=[0.1, 0.3, 0.35, 0.2, 0.05])
        ...
        data['surgery_type'] = np.random.choice([0, 1, 2], n, p=[0.5, 0.3, 0.2])  # 0=minor, 1=major, 2=complex
        
        # 8. Peri-operative parameters
        data['blood_loss'] = np.random.exponential(200, n) * (1 + 0.5 * data['surgery_type'])
        data['transfusion'] = (data['blood_loss'] > 500).astype(int) * np.random.binomial(1, 0.7, n)
        data['hypothermia'] = np.random.binomial(1, 0.15 + 0.1 * data['surgery_duration'] / 100, n)
