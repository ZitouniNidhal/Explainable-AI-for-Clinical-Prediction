...
class RobustnessEvaluator:
    ...
    def add_feature_noise(self, X: pd.DataFrame, feature_noise_rate: float = 0.1) -> pd.DataFrame:
        """
        ...
        for feature in noisy_features:
            ...
            if noise_type == 'gaussian':
                ...
                noise = self.rng.uniform(-0.2 * X[feature].std(), 0.2 * X[feature].std(), X.shape[0])
            else:  # impulsive
                noise = np.zeros(X.shape[0])
                n_impulses = int(0.05 * X.shape[0])
                impulse_indices = self.rng.choice(X.index, n_impulses, replace=False)
                noise[impulse_indices] = self.rng.uniform(-1, 1, n_impulses) * X[feature].std()
            
            X_noisy[feature] = X[feature] + noise
        
        return X_noisy
    
    def compute_jaccard_index(self, features1: List[str], features2: List[str], k: int = 10) -> float:
        """
    ...
    def compute_rank_correlation(self, importance1: np.ndarray, importance2: np.ndarray) -> float:
        """
    ...
    def compute_feature_stability(self, explanations_list: List[Dict]) -> Dict[str, float]:
        """
        ...
        for feature in all_features:
            ...
            if len(importances) > 1:
                feature_stability[feature] = {
                    ...
                    'stability_score': 1 / (1 + np.std(importances))  # Higher is more stable
                }
        
        return feature_stability
    
    def evaluate_comprehensive_robustness(self, X: pd.DataFrame, y: Optional[np.ndarray] = None,
        ...
        for perturbation_type in perturbation_types:
            ...
            for i in range(n_perturbations):
                ...
                if perturbation_type == 'gaussian':
                    ...
                    X_perturbed = self.add_missing_values(X, missing_rate=0.1, missing_pattern='mcar')
                ...
                for feat in baseline_exp['feature'].values:
                    if feat in perturbed_exp['feature'].values:
                        aligned_baseline.append(
                            baseline_exp[baseline_exp['feature'] == feat]['shap_importance'].values[0]
                        )
                        aligned_perturbed.append(
                            perturbed_exp[perturbed_exp['feature'] == feat]['shap_importance'].values[0]
                        )
                
                spearman = self.compute_rank_correlation(
                    np.array(aligned_baseline), 
                    np.array(aligned_perturbed)
                ) if len(aligned_baseline) > 1 else 0.0
            ...
            if perturbation_results:
                ...
                spearman_values = [r['spearman_correlation'] for r in perturbation_results]
                ...
                comprehensive_results['perturbations'][perturbation_type] = {
                    ...
                    'stability_score': np.mean(jaccard_values) * np.mean(spearman_values)
                }
    ...
    def generate_robustness_report(self, results: Dict[str, Any]) -> str:
        ...
        if 'summary' in results:
            summary = results['summary']
            report.append(f"\nOVERALL ASSESSMENT:")
            report.append(f"- Overall Stability Score: {summary['overall_stability']:.3f}")
            report.append(f"- Most Stable Perturbation: {summary['most_stable_perturbation']}")
            report.append(f"- Least Stable Perturbation: {summary['least_stable_perturbation']}")
        ...
        for pert_type, pert_results in results['perturbations'].items():
            ...
            report.append(f"  Jaccard Index: {pert_results['jaccard_mean']:.3f} ± {pert_results['jaccard_std']:.3f}")
            report.append(f"  Spearman Correlation: {pert_results['spearman_mean']:.3f} ± {pert_results['spearman_std']:.3f}")
