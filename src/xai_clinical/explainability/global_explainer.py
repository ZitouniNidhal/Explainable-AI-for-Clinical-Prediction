...
class ClinicalValidator:
    ...
    def stratify_risk(self, probabilities: np.ndarray) -> np.ndarray:
        ...
        medium_risk = (probabilities >= self.risk_thresholds["medium_risk"]) & (
            ~high_risk
        )
    ...
    def generate_global_explanation(
        self, model, X_test: pd.DataFrame, method: str = "shap"
    ) -> Dict[str, Any]:
        """
    ...
    def generate_clinical_report(
        ...
        metric_results = metrics.calculate_all_metrics(
            y_test, (y_pred_proba >= 0.5).astype(int), y_pred_proba
        )

        validation_results = self.validate_clinical_sense(model, X_test)
        explanation = (
            self.generate_global_explanation(model, X_test, method=explanation_method)
        ...
        report.append(f"\nClinical Validation:")
        report.append(
            f"- Age effect valid: {validation_results['age_sensibility']['age_effect_valid']}"
        )
        report.append(
            f"- ASA effect valid: {validation_results['asa_sensibility']['asa_effect_valid']}"
        )
        report.append(
            f"- Emergency effect valid: {validation_results['emergency_sensibility']['emergency_effect_valid']}"
        )
        report.append(
            f"- Overall clinical validity: {validation_results['overall_valid']}"
        )
        ...
        if explanation_method:
            ...
            if explanation_method == "shap":
                for feature, value in zip(
                    explanation["feature_names"], explanation["shap_values"]
                ):
                    report.append(f"- {feature}: {value:.3f}")
            elif (
                explanation_method == "feature_importance"
                and explanation.get("feature_importances") is not None
            ):
                for feature, importance in zip(
                    explanation["feature_names"], explanation["feature_importances"]
                ):
...
class GlobalExplainer:
    ...
    def _permutation_importance(self, X: pd.DataFrame) -> pd.DataFrame:
        ...
        result = permutation_importance(
            self.model,
            X,
            np.zeros(len(X)),  # y n'est pas utilisé pour permutation
            n_repeats=10,
            random_state=42,
        )
