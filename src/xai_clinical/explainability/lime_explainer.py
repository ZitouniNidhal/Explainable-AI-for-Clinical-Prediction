...
class LIMEExplainer:
    ...
    def explain_instance(self, X_instance: Union[pd.Series, np.ndarray], 
        ...
        if isinstance(X_instance, pd.Series):
            ...
            X_array = X_instance.reshape(1, -1)[0] if len(X_instance.shape) > 1 else X_instance
        ...
        return {
            ...
            'prediction_proba': explanation.predict_proba if hasattr(explanation, 'predict_proba') else None,
            'local_pred': explanation.local_pred,
            'intercept': explanation.intercept
        }
    
    def plot_explanation(self, X_instance: Union[pd.Series, np.ndarray],
                        num_features: int = 10,
    ...
    def compare_with_shap(self, X_instance: Union[pd.Series, np.ndarray],
                         shap_explainer,
        ...
        lime_features = {feat.split('=')[0].strip(): weight 
                        for feat, weight in lime_exp['features_weights']}
        
        # SHAP explanation
        X_df = pd.DataFrame([X_instance.values] if isinstance(X_instance, pd.Series) else [X_instance],columns=self.feature_names)
