...
class SHAPExplainer:
    ...
    def _create_explainer(self, X_background: pd.DataFrame):
        ...
        try:
            ...
            if isinstance(self.expected_value, np.ndarray):
                self.expected_value = self.expected_value[1]  # Positive class for binary
    ...
    def explain_local(self, X: pd.DataFrame, instance_idx: int = 0) -> dict:
        ...
        instance_shap = shap_values[instance_idx] if len(shap_values.shape) > 1 else shap_values
        
        # Create result
        explanation = {
            'shap_values': instance_shap,
            'feature_values': X_array[instance_idx] if len(X_array.shape) > 1 else X_array,
    ...
    def explain_global(self, X: pd.DataFrame, max_display: int = 20) -> pd.DataFrame:
        ...
        """
        shap_values = self.explainer.shap_values(X.values if isinstance(X, pd.DataFrame) else X)
    ...
    def plot_dependence(self, X: pd.DataFrame, feature: str, 
        ...
        feature_idx = self.feature_names.index(feature) if feature in self.feature_names else feature
