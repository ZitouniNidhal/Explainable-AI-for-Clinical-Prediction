...
class StabilityAnalyzer:
    ...
    def add_gaussian_noise(self, X: pd.DataFrame, noise_level: float = 0.01) -> pd.DataFrame:
        ...
        X_noisy = X_noisy + noise * X.std()
        return X_noisy
    
    def add_missing_values(self, X: pd.DataFrame, missing_rate: float = 0.05) -> pd.DataFrame:
    ...
    def evaluate_stability(self, X: pd.DataFrame, 
                    ...
                    for feat in baseline_features:
                        if feat in perturbed_shap['feature'].values:
                            aligned_baseline.append(
                                baseline_shap[baseline_shap['feature'] == feat]['shap_importance'].values[0]
                            )
                            aligned_perturbed.append(
                                perturbed_shap[perturbed_shap['feature'] == feat]['shap_importance'].values[0]
                            )
                    
                    if len(aligned_baseline) > 1:
                        jaccard = self.compute_jaccard_index(baseline_features, perturbed_features, top_k)
    ...
    def generate_stability_report(self, results: Dict[str, Any]) -> str:
        ...
        for noise_level, metrics in results['gaussian_noise'].items():
            report.append(f"\n{noise_level}:")
            report.append(f"  Jaccard Index: {metrics['jaccard_mean']:.3f} ± {metrics['jaccard_std']:.3f}")
            report.append(f"  Spearman Correlation: {metrics['spearman_mean']:.3f} ± {metrics['spearman_std']:.3f}")
